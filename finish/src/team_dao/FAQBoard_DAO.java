...
import team_vo.FAQBoard_VO;


public class FAQBoard_DAO {
	...
	public void addData(FAQBoard_VO vo) {
		
		sb.setLength(0);
		sb.append("insert into faqboard ");
		sb.append("values ( faqboard_bno_seq.nextval, ?,?,?,sysdate ) ");
	...
	public ArrayList<FAQBoard_VO> getAllData(int startNo, int endNo){
		ArrayList<FAQBoard_VO> list = new ArrayList<FAQBoard_VO>();
		
		sb.setLength(0);
		sb.append("SELECT * ");
		sb.append("FROM (SELECT ROWNUM,BNO,BTITLE,MNAME,BCONTENTS,BREGDATE ");
		sb.append("FROM (SELECT * FROM FAQBOARD ");
		...
		try {
			...
			while(rs.next()) {
				...
				FAQBoard_VO vo = new FAQBoard_VO(bno, btitle, mname, bcontents, bregdate );
	...
	public FAQBoard_VO getData(int bno) {
		
		sb.setLength(0);
		sb.append("select * from faqboard ");
		sb.append("where bno = ? ");
		...
		try {
			...
			vo = new FAQBoard_VO(bno, btitle, mname, bcontents, bregdate );
	...
	public void modifyData(FAQBoard_VO vo) {
		sb.setLength(0);
		sb.append("update faqboard ");
